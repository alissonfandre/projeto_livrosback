...
//método get retorna todos os editores do banco de dados 
router.get("/",async(req,res) => {
    try{
        //para obter os editores pode-se utilizar .select().orderBy() ou apenas .orderBy()
...
router.post("/",async (req,res)=>{
    ...
    if(!nome || !cidade || !estado || !telefone || !rua|| !cep){
        res.status(400).json({msg:"Enviar nome, estado, telefone, rua, e cep."});
        return;
    }
